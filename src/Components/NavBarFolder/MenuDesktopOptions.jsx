...
const MenuDesktopOptions = ({ nameButton }) => {
    ...
    /*    ESTILOS   */
    const styleMenuOptionsList = 'flex justify-center mx-10';
    const styleSubMenuOptionsList = `${click? 'opacity-100' : 'opacity-0'} transition ease-in-out duration-200 absolute grid grid-cols-2 w-[300px] top-[98%] bg-gradient-to-b from-orange-700 to-orange-400 uppercase border-white text-gray-200 font text-[0.8rem] leading-7 p-2 shadow-[rgba(0,_0,_0,_0.25)_0px_25px_50px_-12px]`;
    ...
    return (
        <div
            ...
            <ul className="flex flex-row leading-10 text-[1rem] font-inter font-normal text-white">
                ...
                <li>
                    <a className={`${styleMenuOptionsList}`} href="#">{nameButton.button1}</a>
                </li>
                    ...
                    <button className={`${styleMenuOptionsList}`} onClick={() => setClick({ men:!click.men })}>
                        ...
                        {click.men && (
                            <div className={`${styleSubMenuOptionsList}`}>
                                {Indumentaria.map((item) => <a className='hover:font-semibold' href="#">{item}</a>)}
                            </div>
                        )}
                    </button>
                </li>
                ...
                <li>
                    <button className={`${styleMenuOptionsList}`} onClick={() => setClick({ brands:!click.brands })}>
                        ...
                        {click.brands && (
                            <div className={`${styleSubMenuOptionsList}`}>
                                {Marcas.map((item) => <a className='hover:font-semibold' href="#">{item}</a>)}
                            </div>)}
                    </button>
                </li>

                <li>
                    <a className={`${styleMenuOptionsList}`} href="#">{nameButton.button5}</a>
                </li>
